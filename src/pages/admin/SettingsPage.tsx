...
import { API_ENDPOINTS } from "../../constants";import AdminLayout from "../../layouts/admin/AdminLayout";
...
const SettingsPage: React.FC = () => {
  ...
  const handleSubmitProfile = useCallback(
    async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const response = await changeCurrentUser.put({body: { fullName, imageUrl: avatar }});
      if (response.ok) {
        console.log("Current user's profile information has been changed!")
      } else {
        console.error("Error while trying to change current user's profile information")
      }
    ...
    <AdminLayout>
      <form onSubmit={handleSubmitProfile}>
        ...
        <div className="relative text-center mt-4 mx-auto block w-max group">
          ...
          />
          <div className="absolite left-0 right-0 bottom-0 mx-auto h-0.5 bg-primary
          transition-all duration-300 ease-out w-0 group-hover:w-full"></div>
        </div>
        {fullNameError && (
